...
def valid_proof(transactions, last_hash, proof):
    ...
    :argument:transactions: The transactions of the block for which the proof is created.
    :argument:last_hash: The previous block's hash which will be stored in the current block.
    :argument:proof: The proof number we're testing.
    """
...
def get_balance(participant):
    ...
    :param participant: The person for whom to calculate the balance.
...
def add_transaction(tx_recipient, tx_sender=owner, tx_amount=1.0):
    ...
    #     "amount": tx_amount
    # }
    transaction = OrderedDict([("sender", tx_sender), ("recipient", tx_recipient), ("amount", tx_amount)])
...
while waiting_for_input:
    ...
    if "1" == user_choice:
        ...
        if len(blockchain) >= 1:
            blockchain[0] = {
                "previous_hash": "",
                "index": 0,
                "transactions": [{"sender": "Chris", "recipient": "Aliaksei", "amount": 100.0}]
            }
    elif "q" == user_choice:
    ...
    if not verify_chain():
        ...
        break
    print("Balance of '{}': {:6.2f}".format("Aliaksei", get_balance("Aliaksei")))
else:
    print("User left!")
